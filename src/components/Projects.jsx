...
// ─── Card heights for layout sizing ───────────────────────────────────────────
const CARD_HEIGHT = 560;   // px — min-h of each card

// ─── Main Component ───────────────────────────────────────────────────────────
export default function Projects() {
  const containerRef = useRef(null);
  const wrapperRefs = useRef([]);   // one ref per card wrapper
  const cardRefs = useRef([]);   // one ref per inner card div
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 2. Individual card entrance — trigger on the WRAPPER, animate the CARD
      wrapperRefs.current.forEach((wrapper, idx) => {
        if (!wrapper) return;
        gsap.from(cardRefs.current[idx], {
          y: 60, opacity: 0, scale: 0.97, duration: 0.75, ease: 'power3.out',
          scrollTrigger: {
            trigger: wrapper,
            start: 'top 90%',
            toggleActions: 'play none none none',
      ...
      // 3. View More button entrance
      gsap.from('.proj-view-more', {
        y: 24, opacity: 0, duration: 0.7, ease: 'power3.out',
        scrollTrigger: {
          trigger: '.proj-view-more',
          start: 'top 93%',
          toggleActions: 'play none none none',
        },
      });
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <div className="proj-header w-full flex flex-col items-center text-center mb-16">
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-12 md:gap-16">
        {projects.map((proj, idx) => (
          <div
            ...
            className="w-full"
          >
            {/* The actual animated card */}
            <div
              ref={el => (cardRefs.current[idx] = el)}
              className="project-card bg-white rounded-[28px] border border-[#D1D5DB] hover:border-[#7C98A4] shadow-[0_6px_30px_rgba(0,0,0,0.05)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.1)] transition-[border-color,box-shadow] duration-500 overflow-hidden flex flex-col md:flex-row w-full"
              style={{ minHeight: CARD_HEIGHT }}
            >
              {/* LEFT: Image */}
              <div className={`${proj.imageBg} md:w-[460px] shrink-0 flex items-center justify-center p-8 md:p-12 min-h-[300px]`} style={{ minHeight: window.innerWidth >= 768 ? CARD_HEIGHT : undefined }}>
                <img
                  src={proj.image}
                  alt={proj.title}
      ...
      {/* ── View More Button ── */}
      <div className="proj-view-more flex justify-center mt-32">
        <a
